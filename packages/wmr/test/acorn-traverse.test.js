...
describe('acorn-traverse', () => {
	it.only("should have 'filename' in plugin state", async () => {
		let filename = '';
		const res = transformWithPlugin(
			dent`
			...
			({ types: t }) => ({
				...
				visitor: {
					Identifier(path, state) {
						if (path.parent.type === 'FunctionDeclaration' && path.node.name !== 'asdfe') {
							path.replaceWith(t.identifier('asdfe'));
						}
					...
					CallExpression(path, state) {
						...
						if (path.node.callee?.name === 'useEffect') {
							path.parentPath.insertAfter(t.assignmentExpression('+', t.identifier('a'), t.identifier('x')));
						}
					}
				}
			}),
			{ filename: 'foobar.js', sourceMap: true }
		);
		...
		await fs.writeFile(path.join(tmp, 'foobar.js'), res.code);
		await fs.writeFile(path.join(tmp, 'foobar.js.map'), JSON.stringify(res.map));

		expect(filename).toEqual('asd.js');
	});
